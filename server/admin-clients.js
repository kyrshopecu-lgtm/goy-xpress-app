...
function syncClient(data,user){const item={id:user.id,userId:user.id,name:user.name,businessName:user.businessName,username:user.username||'',whatsapp:user.phone,phone:user.phone,email:user.email||'',documentId:user.documentId||'',address:user.address||'',mapUrl:user.mapUrl||'',location:user.location||null,logo:user.logo||'',approved:true,active:user.active!==false,status:user.active===false?'Inactivo':'Activo',registeredAt:user.createdAt};const i=data.clients.findIndex(x=>x.userId===user.id||x.id===user.id);if(i>=0)data.clients[i]={...data.clients[i],...item};else data.clients.unshift(item);}
  ...
  return async function handler(req,res){
    ...
    try{
      ...
      if(req.method==='POST'&&!match[1]){
        const body=await readBody(req);const email=normalizeEmail(body.email),username=normalizeUsername(body.username),phone=cleanPhone(body.phone||body.whatsapp),name=String(body.name||'').trim(),businessName=String(body.businessName||'').trim(),documentId=String(body.documentId||'').trim(),address=String(body.address||'').trim(),mapUrl=String(body.mapUrl||'').trim(),logo=String(body.logo||'');
        ...
        const user={id:crypto.randomUUID(),role:'client',name,businessName,username,email,phone,documentId,address,mapUrl,location,logo,approved:true,active:true,passwordSalt:pwd.salt,passwordHash:pwd.hash,createdAt:now,updatedAt:now,createdBy:'admin'};
