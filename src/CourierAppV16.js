...
function Field({label,...p}){return <View style={s.field}><Text style={s.label}>{label}</Text><TextInput {...p} placeholderTextColor="#78909B" style={s.input}/></View>}
function Btn({title,onPress,disabled=false,outline=false}){return <Pressable disabled={disabled} onPress={onPress} style={[s.btn,outline&&s.outline,disabled&&{opacity:.5}]}><Text style={[s.btnText,outline&&s.outlineText]}>{title}</Text></Pressable>}
function Auth({onOk}){
  const[mode,setMode]=useState('login'),[busy,setBusy]=useState(false),[photo,setPhoto]=useState(''),[f,setF]=useState({name:'',phone:'',email:'',password:''});
  const set=(key,value)=>setF(current=>({...current,[key]:value}));
  const choose=async camera=>{try{const value=await pickCourierPhoto(camera);if(value)setPhoto(value)}catch(error){Alert.alert('Foto',error.message)}};
  const submit=async()=>{
    if(!f.email.trim()||!f.password)return Alert.alert('Faltan datos',mode==='register'?'Ingresa correo y contraseña.':'Ingresa usuario/correo y contraseña.');
    ...
    try{
      if(mode==='register'){
        ...
        Alert.alert('Registro enviado','Administración debe aprobar tu cuenta antes de habilitar tus rutas.');
        setMode('login');
        return;
      }
      const result=await login('courier',f.email.trim(),f.password);
      await AsyncStorage.setItem(KEY,JSON.stringify({token:result.token}));
      onOk(result.token);
    }catch(error){Alert.alert('GOY XPRESS',error.message)}finally{setBusy(false)}
  };
  return <SafeAreaView style={s.safe}><StatusBar style="light" backgroundColor={C.navy}/><ScrollView contentContainerStyle={s.page}><Image source={require('../assets/goy-logo.jpg')} style={s.logo}/><Text style={s.kicker}>MENSAJERO GOY XPRESS</Text><Text style={s.title}>GOY XPRESS</Text><View style={s.panel}><View style={s.segment}><Pressable onPress={()=>setMode('login')} style={[s.seg,mode==='login'&&s.segOn]}><Text style={s.segText}>Ingresar</Text></Pressable><Pressable onPress={()=>setMode('register')} style={[s.seg,mode==='register'&&s.segOn]}><Text style={s.segText}>Registrarme</Text></Pressable></View>{mode==='register'?<><View style={s.photoRow}>{photo?<Image source={{uri:photo}} style={s.avatar}/>:<View style={s.avatarEmpty}><Text style={s.avatarText}>GX</Text></View>}<View style={{flex:1}}><Btn title="Elegir foto" outline onPress={()=>choose(false)}/><Btn title="Tomar foto" outline onPress={()=>choose(true)}/></View></View><Field label="Nombre completo" value={f.name} onChangeText={value=>set('name',value)}/><Field label="WhatsApp" keyboardType="phone-pad" value={f.phone} onChangeText={value=>set('phone',value)}/></>:null}<Field label={mode==='register'?'Correo':'Usuario o correo'} keyboardType={mode==='register'?'email-address':'default'} autoCapitalize="none" value={f.email} onChangeText={value=>set('email',value)}/><Field label="Contraseña" secureTextEntry autoCapitalize="none" value={f.password} onChangeText={value=>set('password',value)}/><Btn title={busy?'Procesando…':mode==='register'?'Enviar registro':'Ingresar'} disabled={busy} onPress={submit}/></View></ScrollView></SafeAreaView>
}
