...
function calculateExecutivePrice(minutes) {
  ...
  const extraMinutes = Math.max(
    0,
    requestedMinutes - PRICING.executiveIncludedMinutes,
  );
  const surcharge = roundMoney(extraMinutes * PRICING.executiveExtraMinute);

  return {
    requestedMinutes,
    includedMinutes: PRICING.executiveIncludedMinutes,
    extraMinutes,
    surcharge,
    total: roundMoney(PRICING.executiveBase + surcharge),
  };
}
...
function calculateDeliveryPrice(mode, distanceKm) {
  ...
  if (mode === 'scheduled') {
    return {
      mode,
      distanceKm: distance,
      includedKm: PRICING.expressIncludedKm,
      extraKm: Math.max(0, Math.ceil(distance - PRICING.expressIncludedKm)),
      total: PRICING.scheduledDelivery,
      eligible: distance > 0 && distance <= PRICING.expressIncludedKm,
    };
  }

  const extraKm = Math.max(
    0,
    Math.ceil(distance - PRICING.expressIncludedKm),
  );
  return {
    mode: 'express',
    distanceKm: distance,
    ...
    extraKm,
    total: roundMoney(
      PRICING.expressBase + extraKm * PRICING.expressExtraKm,
    ),
    eligible: distance > 0,
  };
}

function calculateCollectTotal({
  productValue,
  deliveryCost,
  cashOnDelivery,
  deliveryPayer,
  ...
  const product = nonNegativeNumber(productValue);
  const delivery = nonNegativeNumber(deliveryCost);
  return roundMoney(
    product + (deliveryPayer === 'recipient' ? delivery : 0),
  );
}
...
function createCode(kind, now = Date.now(), random = Math.random()) {
  ...
  const randomPart = Math.floor(random * 1000).toString().padStart(3, '0');
...
function normalizeRequest(request) {
  ...
  return {
    ...request,
    status: statusMap[request?.status] || REQUEST_STATUS.pending,
    serviceCost: nonNegativeNumber(request?.serviceCost),
    totalToCollect: nonNegativeNumber(request?.totalToCollect),
  };
}
