...
function normalizeEcuadorPhone(value) {
  ...
  if (String(value || '').trim().startsWith('+') && raw.length >= 10) return `+${raw}`;
  return '';
}
...
function validateRegistration(profile) {
  ...
  if (!validateDocument(documentType, documentNumber)) {
    errors.push(documentType === DOCUMENT_TYPES.ruc ? 'El RUC debe tener 13 dígitos.' : 'La cédula debe tener 10 dígitos.');
  }
  ...
  return {
    ...
    errors,
    value: {fullName,address,whatsapp,contactPhone,documentType,documentNumber,email},
  };
}
