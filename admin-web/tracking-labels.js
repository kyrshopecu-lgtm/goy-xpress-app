(() => {
  ...
  function labelHtml(request) {
    ...
    <style>
      *{box-sizing:border-box}body{margin:0;background:#eef4f6;font-family:Arial,Helvetica,sans-serif;color:#122b36}.page{width:100%;max-width:760px;margin:8px auto;background:#fff;border:1.5px solid #0b2f40;border-radius:12px;overflow:hidden}.head{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 14px;background:#0b2f40;color:#fff}.brand{display:flex;align-items:center;gap:9px}.brand img{width:48px;height:48px;object-fit:cover;border-radius:9px;background:#fff}.brand strong{display:block;font-size:18px}.brand span{font-size:10px;opacity:.8}.tracking{text-align:right}.tracking small{display:block;text-transform:uppercase;letter-spacing:.1em;font-size:8px;opacity:.75}.tracking b{display:block;font-size:18px;letter-spacing:.03em;margin-top:3px}.body{padding:10px 14px}.codes{display:grid;grid-template-columns:126px 1fr;gap:10px;align-items:center;border:1px solid #0b2f40;border-radius:10px;padding:8px;margin-bottom:8px}.qr-box{text-align:center}.qr-box #qrcode{display:inline-block;padding:4px;background:#fff}.qr-box small,.barcode-box small{display:block;margin-top:3px;color:#68808b;font-size:9px;font-weight:700}.barcode-box{text-align:center;overflow:hidden}.barcode-box svg{max-width:100%;height:auto}.notice{background:#eaf8fc;border:1px solid #c9ebf3;border-radius:8px;padding:6px 8px;margin-bottom:8px;font-size:10px;font-weight:700;color:#0b708b}.grid{display:grid;grid-template-columns:1fr 1fr;gap:6px}.card{border:1px solid #dce8ec;border-radius:8px;padding:6px 8px;min-height:46px}.card span{display:block;color:#78909c;text-transform:uppercase;font-size:8px;font-weight:800;letter-spacing:.06em;margin-bottom:2px}.card strong{font-size:11px;line-height:1.2;overflow-wrap:anywhere}.wide{grid-column:1/-1}.route{margin-top:8px;border-top:1.5px dashed #cbd8dd;padding-top:7px}.route h2{font-size:12px;margin:0 0 4px}.route-item{padding:5px 0;border-bottom:1px solid #e8eff2}.route-item span{display:block;font-size:8px;text-transform:uppercase;color:#7d919a;font-weight:800}.route-item strong{display:block;margin-top:2px;font-size:11px;line-height:1.2}.foot{display:flex;justify-content:space-between;gap:10px;align-items:flex-end;margin-top:8px;padding-top:7px;border-top:1px solid #e5ecef}.foot small{color:#78909c;font-size:8px}.amount{text-align:right}.amount span{display:block;color:#78909c;font-size:8px;text-transform:uppercase}.amount strong{font-size:15px}.actions{max-width:760px;margin:8px auto 14px;text-align:center}.actions button{border:0;border-radius:9px;padding:9px 14px;font-weight:800;background:#00a9e8;color:#fff;cursor:pointer}.code-status{font-size:9px;color:#78909c;margin-top:4px}@media print{html,body{width:210mm;height:297mm;background:#fff}body{overflow:hidden}.page{width:196mm;max-width:196mm;margin:0 auto;border-radius:0;border-width:1px;page-break-inside:avoid;break-inside:avoid}.actions{display:none}.head{padding:8px 12px}.body{padding:8px 12px}.brand img{width:42px;height:42px}.codes{grid-template-columns:112px 1fr;gap:8px;padding:6px;margin-bottom:6px}.notice{padding:5px 7px;margin-bottom:6px}.grid{gap:5px}.card{padding:5px 7px;min-height:42px}.route{margin-top:6px;padding-top:6px}.route-item{padding:4px 0}.foot{margin-top:6px;padding-top:5px}@page{size:A4 portrait;margin:6mm}}@media(max-width:620px){.head{align-items:flex-start;flex-direction:column}.tracking{text-align:left}.codes{grid-template-columns:110px 1fr}.grid{grid-template-columns:1fr 1fr}.page{margin:0;border-radius:0;border-width:0}.body{padding:9px}}
    </style></head><body><main class="page"><header class="head"><div class="brand"><img src="${escapeHtml(location.origin)}/assets/goy-logo.jpg" alt="GOY XPRESS"><div><strong>GOY XPRESS</strong><span>Guía de entrega y seguimiento</span></div></div><div class="tracking"><small>Número de tracking</small><b>${escapeHtml(tracking)}</b></div></header><section class="body"><div class="codes"><div class="qr-box"><div id="qrcode"></div><small>QR de seguimiento</small></div><div class="barcode-box"><svg id="barcode"></svg><small>Código de barras CODE 128</small><div class="code-status" id="codeStatus">Generando códigos…</div></div></div><div class="notice">El QR y el código de barras identifican esta orden con el mismo número de tracking.</div><div class="grid"><div class="card"><span>Cliente</span><strong>${escapeHtml(clientName)}</strong></div><div class="card"><span>WhatsApp</span><strong>${escapeHtml(clientPhone)}</strong></div><div class="card"><span>Cédula / RUC</span><strong>${escapeHtml(documentId)}</strong></div><div class="card"><span>Correo</span><strong>${escapeHtml(clientEmail)}</strong></div><div class="card wide"><span>Dirección registrada del cliente</span><strong>${escapeHtml(clientAddress)}</strong></div></div><div class="route"><h2>Datos de la orden</h2>${origin ? `<div class="route-item"><span>Retiro</span><strong>${escapeHtml(origin)}</strong></div>` : ''}<div class="route-item"><span>Dirección / destino</span><strong>${escapeHtml(destination)}</strong></div><div class="route-item"><span>Persona que recibe</span><strong>${escapeHtml(recipient)}</strong></div><div class="grid" style="margin-top:6px"><div class="card"><span>Orden</span><strong>${escapeHtml(code || '-')}</strong></div><div class="card"><span>Servicio</span><strong>${escapeHtml(serviceName(request))}</strong></div><div class="card"><span>Estado</span><strong>${escapeHtml(statusName(request.status))}</strong></div><div class="card"><span>Mensajero</span><strong>${escapeHtml(request.courier || 'Sin asignar')}</strong></div></div></div><div class="foot"><small>Generado: ${escapeHtml(formatDate(request.createdAt || request.updatedAt))}<br>GOY XPRESS · Quito, Ecuador</small><div class="amount"><span>Valor del servicio</span><strong>${escapeHtml(money(request.serviceCost ?? request.value ?? 0))}</strong></div></div></section></main><div class="actions"><button onclick="window.print()">Imprimir guía</button></div>
    <script>
      ...
      function renderCodes(){
        ...
        if(window.QRCode && window.JsBarcode){
          try{
            new QRCode(document.getElementById('qrcode'),{text:qrPayload,width:104,height:104,correctLevel:QRCode.CorrectLevel.M});
            JsBarcode('#barcode',tracking,{format:'CODE128',displayValue:true,fontSize:13,height:46,margin:3});
